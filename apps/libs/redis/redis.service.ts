...
export class RedisService extends Redis implements OnModuleDestroy {
  ...
  private handleError(err: any) {
    ...
    if (isDevelopment) {
      this.logger.error('Redis error occurred! Make sure to read the README.md file to fix this.', {
        type: 'REDIS_ERROR',
        err,
      });
    } else {
   ...
   * @param ttl
   */
  async compareAndSwap(key: string, current: string, next: string, ttl?: number): Promise<boolean> {
    const casLua = `     
      local key = KEYS[1]
  ...
  //   return this.set(key, value, { ttl } as any);
  // }
